...
void ui_wait_for_click(void)
{
    do ; while (ioport_get_pin_level(BUTTON_0_PIN) == BUTTON_0_INACTIVE);
    do ; while (ioport_get_pin_level(BUTTON_0_PIN) == BUTTON_0_ACTIVE);
}

// Warn about flashing bootlader.
void ui_warn_bl_flash(void)
{
    LED_Off(LED0);
    led_pulses = 2;
    led_pause = 14;
